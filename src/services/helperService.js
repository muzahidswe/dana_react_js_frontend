...
const GET_LOCALTION_URL_FOR_REPORT = baseURL + "get-location-based-on-permission-for-report";
const GET_LOCALTION_URL_DD_TWO = baseURL + "get-locations-fi-di-wise";
...
export async function getLocationsForReport() {
  try {
    const user_id = localStorage.getItem("id");
    const token = localStorage.getItem("token");
    const user_type = localStorage.getItem("cr_user_type");
    ...
    const locations = await axios.post(GET_LOCALTION_URL_FOR_REPORT, json_obj, {
        headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
            Authorization: "Bearer " + token
...
export async function getLocationsForDropDownTwo() {
  try {
    const user_id = localStorage.getItem("id");
    const token = localStorage.getItem("token");
    const user_type = localStorage.getItem("cr_user_type");
    ...
    const locations = await axios.post(GET_LOCALTION_URL_DD_TWO, json_obj, {
        headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
            Authorization: "Bearer " + token
