...
import {  fileSubmitSupervisorNIDInfoUpload } from '../../../services/fileUploadServices';
...
import SupervisorDIstributoreModal from "./SupervisorDeleteModal";
...
function SupervisorInfoUpload(props) {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { setStatus, setSubmitting }) => {   
            ...
            setSubmitting(true);
            setLoading(true);
            fileSubmitSupervisorNIDInfoUpload(document.getElementById('uploadFile').files[0]).then((val)=>{
                if (val.data.success) {
    ...
    const sampleDownload ='/assets/download/Supervisor-Sample.xlsx';
        ...
        <Card className="m-5">
            <Card.Body>
                    <form onSubmit={formik.handleSubmit} autoComplete="off" className="form">
                        <div className="row">
                            <div className="col-3">
                                <h4 className="card-title">Upload Supervisor Information</h4>
                            </div>
                            <div className="col-4">
                                            ...
                                            {data && data.length>0 &&  data?.map((data, index) => {
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td className={styles.valueText}>{data?.supervisor_name}</td>
                                                        <td className={styles.valueText}>{data?.supervisor_nid}</td>
                                                        <td className={styles.valueText}>{data?.phone}</td>
                                                        <td className={styles.valueText}>{data?.manufacturer_id}</td>
                                                        <td className={styles.valueText}>{data?.supervisor_employee_code}</td>
...
export default SupervisorInfoUpload;