...
function DropdownMenuGroup(props) {
    ...
    const getPointIds = () => {
        ...
        if (selectedPhase.length > 0) {
            selectedPhase.forEach(e => {
                phases.push(e.value);
            })
        }

        props.onDpidChange(dpids);
        props.onPhaseChange(phases);
    }

    const onChangeRegion = (val) => {
        if (val.length > 0) {
            //   console.log(areaByRegionRepo,houseByRegionRepo,pointByRegionRepo);
            var tempArea = [];
            var tempHouse = [];
            var tempTerritory = [];
            var tempPoint = [];
    ...
    const onChangeHouse = (val) => {
        ...
        if (val.length > 0) {
            ...
            console.log(tempTerritory, tempPoint);
    ...
    useEffect(() => {
        (async () => {
            ...
            if (!locations) {
                ...
                var tempPhases = [];
                locations?.phases?.forEach(e => {
                    var temp = {
                        label: e.name,
                        value: e.id
                    };
                    tempPhases.push(temp);
                })
                ...
                <Card className="bg-light-primary">
                    <Card.Body className="m-5">
                        <div className={styles.filterWrapper}>
                            ...
                            <div style={(userType == 'fi') ? {display:"none"} : {}}>
                                <div className="row">
                                    <div className="col-md-12">
                                        <h6>Territory</h6>
                                    </div>
